...
import com.miqtech.wymaster.wylive.module.live.ShareWeiboActivity;
...
//    }
    ...
    IUiListener qqShareListener = new IUiListener() {
        ...
        public void onError(UiError e) {
            // TODO Auto-generated method stub
            ToastUtils.show("分享到QQ失败" + e.errorMessage + " 错误码" + e.errorCode);
            if (mSharePopWindow != null) {
                mSharePopWindow.dismiss();
            }
        }
    ...
    public void reqMsg(final String token) {
        if (shareWeiboBitmap != null) {//微博分享的是图片，后面代码就不执行
            ...
            if (!accessToken.getToken().equals("")) {
                WeiboMultiMessage weiboMessage = new WeiboMultiMessage();//初始化微博的分享消息
                mWeiboAuth = new AuthInfo(mContext, Constants.WB_APP_KEY, Constants.WB_REDIRECTURL, Constants.WB_APP_SCOPE);
                ImageObject imageObject = new ImageObject();
                TextObject textObject = new TextObject();
...
//                mSsoHandler.authorize(new AuthListener());
    ...
    public IWeiboShareAPI getIWeiApiInstance(Context mcontext) {
        if (weiboApi == null)
            weiboApi = WeiboShareSDK.createWeiboAPI(mcontext, Constants.WB_APP_KEY);
        return weiboApi;
    }
...
//    }
    ...
    private void sendShareHttp() {
        String shareTask = API.HOST + API.AFTER_SHARE;
      ...
      //  Requestutil.sendPostRequest(builder.append(url).toString(), params, url, this, TAG);
        Requestutil.sendPostRequest(builder.append(API.AFTER_SHARE).toString(),params,API.AFTER_SHARE,this,mContext.getClass().getName());
        //Requestutil.mRequest.excutePostRequest(shareTask, params, this, HttpConstant.AFTER_SHARE);
     //   L.e("params", "params : " + params.toString());
    }
