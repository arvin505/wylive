...
import com.miqtech.wymaster.wylive.module.comment.adapter.PersonalCommentDetailAdapter;
...
import com.miqtech.wymaster.wylive.widget.MyListView;
import com.miqtech.wymaster.wylive.widget.pullToRefresh.PullToRefreshBase;
import com.miqtech.wymaster.wylive.widget.pullToRefresh.PullToRefreshScrollView;
 ...
 */
@LayoutId(R.layout.activity_personal_comment_detail)
public class PersonalCommentDetail extends BaseAppCompatActivity implements View.OnClickListener,
        PersonalCommentDetailAdapter.DealWithData {

    @BindView(R.id.refresh_personal_comment)
    ...
    @BindView(R.id.comment_top_title_ll_item)
...
//                context.startActivity(intent);
    ...
    private void requestData() {
       // showLoading();
        ...
        sendHttpRequest(API.V2_AMUSE_COMMENT_LIST,map);
    }
        ...
        try {
            if (method.equals(API.V2_AMUSE_COMMENT_LIST)) {
              ...
              //  BroadcastController.sendUserChangeBroadcase(context);
                ...
                isRefre = 1;
            } else if (method.equals(API.DEL_COMMENT)) {//删除
                if (mDialog != null) {
                    mDialog.dismiss();
                }
                showToast("删除成功");
    ...
    public void onFaild(JSONObject object, String method) {
     ...
     //   hideLoading();
        ...
        try {
            if (method.equals(API.AMUSE_COMMENT)) {
    ...
    private void analysisJSONData(JSONObject object) {
        ...
        try {
            ...
            if (page == 1) {
                ...
                FirstCommentDetail newFirstCommentDetail = gs.fromJson(object.getString("parent").toString().trim(),FirstCommentDetail.class);
                firstCommentDetail = newFirstCommentDetail;
                setTitle(newFirstCommentDetail.getNickname()+"的评论");
                showCommentData();
            ...
            List<SecondCommentDetail> newSecondCommentDetailList =gs.fromJson(object.getString("list").toString().trim(),new TypeToken<List<SecondCommentDetail>>(){}.getType());
    ...
    private void addReplyReply(String object) {
        Gson gs =new Gson();
        SecondCommentDetail bean =gs.fromJson(object,SecondCommentDetail.class);
...
//    }
        ...
        TextWatcher textWatcher = new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (!TextUtils.isEmpty(myTemp)) {
                    tvSend.setTextColor(getResources().getColor(R.color.bar_text_selected));
                } else {
                    tvSend.setTextColor(getResources().getColor(R.color.category_item_title));
                }
    ...
    private void showCommentData() {
        ...
        if (!TextUtils.isEmpty(firstCommentDetail.getIcon())) {
            ...
            AsyncImage.displayImage(firstCommentDetail.getIcon() , ivUserHeader,R.drawable.default_head);
        } else {
        ...
        if (!TextUtils.isEmpty(firstCommentDetail.getCreateDate())) {
            strDate = DateUtils.friendlyTime(firstCommentDetail.getCreateDate());
